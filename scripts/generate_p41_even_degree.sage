"""
generate_p41_even_degree.sage — Generate 2 random genus-g hyperelliptic curve
test cases for each genus g = 1..30, with even-degree f (deg = 2g+2) over GF(41).
...
Run from repo root:
    sage scripts/generate_p41_even_degree.sage
"""
...
for g in range(1, MAX_GENUS + 1):
    ...
    for i in range(1, N_PER_GENUS + 1):
        ...
        try:
            C = random_hyperelliptic_even_degree(P, g)
            t0 = time.time()
            ...
            print("[p41-even] g={:2d}, case {}/{}: {} ({:.2f}s)".format(
                g, i, N_PER_GENUS, case_id, elapsed))
            sys.stdout.flush()
            if elapsed > TIME_LIMIT:
                print("[p41-even] Exceeded time limit ({:.2f}s > {}s). Stopping at genus {}.".format(
                    elapsed, TIME_LIMIT, g))
                stop = True
                break
        except Exception as e:
            print("[p41-even] ERROR on {}: {}".format(case_id, e), file=sys.stderr)
            sys.stderr.flush()
...
print()
print("[p41-even] Done. Output written to {}".format(FILENAME))
